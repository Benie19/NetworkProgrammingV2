...
# demo key (must match sender/relay)
...
def fetch_messages(destination='rover2'):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect(server_address)
    fetch_request = {"status": "fetch", "destination": destination}
    # framed send
    ...
    while True:
        ...
        if compute_checksum(plaintext) != msg.get('checksum'):
            ...
            continue
        print(f"Received MWRS Message id={msg.get('id')} from={msg.get('from')}: {plaintext.decode('utf-8')}")
    client_socket.close()
